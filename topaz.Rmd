...
op_16_wmem <- \(vec, loc) { # expect numeric vector of length 2 that breaks into a, b
  #allegedly b will reliably be a register address this time. #lmao nope
  a <- do_place(vec[1])
...
op_18_ret <- \(loc) {
  ...
  #  memory[z == 32767, `:=` (done = FALSE,
    ...
    print(paste0("18 ret at ", 
                 as.character(loc), 
                 " found a non-empty stack; moving."))
    place <- meta_pop() # |> do_place() # is this needed?
#    find(place)
    print(paste("Location found is:", place))
    if (isFALSE(place == chk_RETLOG)) {
      chk_RETLOG <<- place
    } else {
      rets <<- c((rets[1] + 1), place)
    }
    finish()
    do_next <<- place
#    counteR()
