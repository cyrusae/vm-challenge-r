...
in: 20 a read a character from the terminal and write its ascii code to
...
from the keyboard and trust that they will be fully
...
counteR <- function(from) {
  c <- memory[z == (from), cmd]
  memory[z == (from), do := TRUE]
  i <- from + 1
  if (isFALSE(c %in% takes0)) {
    memory[z == (i), do := FALSE] #if we take one or more arguments, mark the immediate next line
  }
  if (isTRUE(c %in% c(takes2, takes3))) {
    i <- i + 1
    memory[z == (i), do := FALSE] #mark second argument if two arguments are taken
  }
...
find <- function(after = 0) { #default to starting from z=0
  ...
  do_next <<- memory[between(cmd, 0, 21), ][(is.na(done) | done == FALSE), ][z >= (after), z][1] |> as.integer() #should get the first of remaining commands?
  counteR(do_next)
...
#  memory[(z == (loc)), done := TRUE] #mark the command with this z-index as done
  ...
  memory[between(z, (loc), (nnext)), done := TRUE]
  return(invisible(NULL))
}
