...
op_07_jt <- \(vec, loc) {
  ...
  b <- vec[2]
  if (isTRUE(a > 0)) {
    print(paste("jt: attempting jump based on value of <a> = ", as.character(a), ", from", as.character(loc)))
    op_06_jmp(b, loc)
  } else {
    finish()
  }
  print(paste("Moving to", as.character(do_next)))
  return(invisible(NULL))
}
...
op_08_jf <- \(vec, loc) {
  ...
  if (isTRUE(a < 1)) {
    print(paste("jf: attempting jump based on value <a> =", as.character(a), "from", as.character(loc)))
    op_06_jmp(b, loc)
  } else {
    finish()
  }
...
#    find(do_next)
  ...
  print("Counter set to 32768; program self-terminates.")
  return(invisible(NULL))
}
```
