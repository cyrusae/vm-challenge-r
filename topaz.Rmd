...
do_place <- function(x) {  #either interpret a literal value, or read from the register 
  if (isTRUE(x >= 32768)) {
    ...
    print(paste("do_place retrieved", as.character(a), "from the registers at position z =", as.character(xval)))
    a
  } else {
    a <- interpret(x)
...
do_write <- function(value, to) {
  if (isTRUE(to < 32768)) {
    ...
    print(paste("do_write wrote value", value, "to registers at address z =", toval))
  }
  return(invisible(NULL))
}
...
op_02_push <- \(a, loc) { #expect a and loc
  aval <- interpret(a)
  stack <<- c(aval, stack) #taking a here literally until proven otherwise on the assumption that if it wants to undergo do_place() that'll happen later.
  print(paste("Pushing value", as.character(aval), "(aka", as.character(a),"modulo 32768) onto the stack"))
#  stack <<- c(a, stack)
...
op_03_pop <- \(a, loc) {
#  if (isTRUE(length(stack) < 2)) {
#    print("Skipping pop due to single-item stack.")
#    finish()
#  } else {
    obj <- meta_pop()
    print(paste("Popping value", as.character(obj), "out of the stack and into position", as.character(a)))
    do_write(obj, a)
...
op_04_eq <- \(vec, loc) {
  ...
  c <- do_place(vec[3])
  if (identical(b, c)) {
    print(paste0("op 4 (eq) found <b> = <c> = ", as.character(b)))
    do_write(1, a)
  } else {
    print(paste0("op 4 (eq) found <b> = ", as.character(b), " and <c> = ", as.character(c)))
    do_write(0, a)
  }
  finish()
  return(invisible(NULL))
...
op_05_gt <- \(vec, loc) {
  ...
  b <- do_place(vec[2])
  c <- do_place(vec[3])
  print(paste0("op 5 (gt) found <b> = ", as.character(b), "and <c> = ", as.character(c)))
  if (b > c) {
    do_write(1, a)
  } else {
    do_write(0, a)
...
#  b <- vec[2] #remember op 6 handles 
  if (isTRUE(a < 1)) {
    print(paste("jf: attempting jump to <b> = ", as.character(b), "based on value <a> =", as.character(a), "from", as.character(loc)))
#    op_06_jmp(b, loc)
    finish()
    do_next <<- b
  } else {
...
op_09_add <- \(vec, loc) {
  ...
  b <- do_place(vec[2])
  c <- do_place(vec[3])
  print(paste0("9 add: adding <b> = ", as.character(b), " + <c> = ", as.character(c)))
  got <- (b + c) |> interpret()
  do_write(got, a)
  finish()
  return(invisible(NULL))
...
op_10_mult <- \(vec, loc) {
  ...
  b <- do_place(vec[2])
  c <- do_place(vec[3])
  print(paste0("10 mult: multiplying <b> = ", as.character(b), " * <c> = ", as.character(c)))
  got <- (b * c) |> interpret()
  do_write(got, a)
  finish()
  return(invisible(NULL))
...
op_11_mod <- \(vec, loc) {
  ...
  b <- do_place(vec[2])
  c <- do_place(vec[3])
  print(paste0("11 mod: <b> = ", as.character(b), " %% <c> = ", as.character(c)))
  got <- (b %% c) # |> interpret() #???
  do_write(got, a)
  finish()
  return(invisible(NULL))
...
op_15_rmem <- \(vec, loc) { #expect numeric vector of length 2 that breaks into a, b
  ...
  print(paste0("15 rmem using z = ", as.character(aval), " (<a>) and value <b> = ", as.character(bval)))
#  a <- vec[1]
  do_write(bval, aval)
  finish()
  return(invisible(NULL))
...
op_16_wmem <- \(vec, loc) { # expect numeric vector of length 2 that breaks into a, b
  ...
  print(paste0("16 wmem using z = ", as.character(a), " (<a>) and source <b> = ", as.character(bval)))
...
op_17_call <- \(a, loc) {
  ...
  print(paste("op 17 (call) wrote", as.character(write_next), "to the stack at line", as.character(loc), ". Jumping to <a> = ", as.character(a)))
  op_6_jmp(a, loc)
  return(invisible(NULL))
}
...
op_18_ret <- \(loc) {
  ...
  #  memory[z == 32767, `:=` (done = FALSE,
    ...
    print(paste0("18 ret at ", as.character(loc), " found a non-empty stack; moving."))
...
op_20_in <- \(a, loc) {
  op20prompt <- paste0("Enter input for op 20 triggered at z = ", as.character(loc))
  ...
  print(paste0("Last character from input string (", str[[i]], ") written to z = ", as.character(go)))
  finish()
  return(invisible(NULL))
}
...
sendeR <- function(memory) {
  ...
  if (!is.null(stack) & (length(stack) > 0)) {
    print(paste("As of line", do_next, "the stack contains", length(stack), "items"))
  }
  if (do_this != 19) {
    if (!is.null(printqueue) | isTRUE(length(printqueue) > 0)) {
      cat(printqueue, sep = "")
      print("[End printing.]")
      printqueue <<- c()
    }
    print(paste0("[NOW PERFORMING: ", as.character(do_this), " as of z = ", as.character(do_next), "]."))
  }
  if (do_this == 0) {
    op_00_halt(do_next)
  } 
...
understandeR <- function(cmds = cmdlist) { #comma-separated list of integers unbinaried from binary goes here
  ...
  print("Counter reached 32768; program ends.") #log last known iteration 
  return(invisible(NULL))
}
```
