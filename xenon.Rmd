...
write_loc <- \(value, to = NULL) {
  ...
  if (to >= modulo) { 
    ...
    reglog <- paste0("**Register change:** writing value = ",
                     value, " to = ", to)
    writeLines(reglog)
    loglines <- append(loglines, " ", "---", " ", reglog, " ")
  }
...
dispatcheR <- \() {
  ...
  if (!(perform %in% c(0, 19, 21))) { 
    ...
    writeLines(c(paste0("Doing: ", 
                   .subset2(opnames, (perform + 1)), 
                   " at z = ", doing)), 
               paste0("[work] contains:", as.character(work)))
    if (identical(args, work)) {
      writeLines("and [args] match [work].")
    } else {
      writeLines(paste0("[args] contains:", as.character(args)))
    }
    print("...")
  }
