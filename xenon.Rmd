...
start <- \(loc) { 
  work <- memory[z %in% ((loc)+1):((loc)+3), cmd]
  work
}
...
dispatcheR <- \() {
  ...
  if ((length(printqueue) > 0) & 
      ...
      (length(printqueue) > 50) & (perform != 19)) { 
  ...
  if (perform > 21) { 
    print(paste0(.subset2(opnames, (perform + 1))))
    print(perform)
    print(doing)
    print(work)
    print(args)
...
```{r starteR}
starteR <- \(resume = FALSE, play = FALSE, from = NA, action = NA, write = NA) {
  if (isTRUE(resume)) {
    restore()
  } else {
    scratch()
