...
dispatcheR <- \() {
  ...
  if ((length(printqueue) > 0) & 
      ...
      (length(printqueue) > 50) & !(perform %in% c(6:8, 17:19))) { 
  ...
  if (perform > 21 & perform > 0) { 
    print(c(paste0("Doing: ", .subset2(opnames, (perform + 1)), " at z = ", doing),
            paste0("[work] contains: ", work)))
    if (identical(args, work)) {
      print("[args] match [work].")
    } else {
