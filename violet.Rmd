...
pop_stack <- \() {
  ...
  stack <<- stack[-1]
  ###::
  loglines <<- c(loglines, (paste0("x = ", x, " was popped out of the stack! Stack contains: ")), "", 
                 paste(stack, sep = ", "), "")
  x
}
...
dispatcheR <- \() {
  ...
  if (perform != 19 & length(printqueue) > 0) {
    msg <- c("[MESSAGE:", capture.output(cat(printqueue, sep = "")), "[END MESSAGE]", "")
    writeLines(msg)
...
op_00_halt <- \(...) { #take and ignore arbitrary arguments
  loglines <- c(loglines, (paste0("HALT :: Program halted by z-index ", doing)))
  finish(doing)
  doing <<- 32768
  return(invisible(NULL))
}
...
op_07_jt <- \(...) {
  ...
  ###::
  loglines <<- c(loglines, 
                 (paste0("!!:: opcode 7 jt called by doing = ",
                         doing, " with <a> = ", a, 
...
op_08_jf <- \(...) {
  ...
  ###::
  loglines <<- c(loglines, 
                 (paste0("!!:: opcode 7 jf called by doing = ",
                         doing, " with <a> = ", a, 
                         " as read by use_integer().")))
  finish(doing, 2)
  if (a == 0) {
...
op_15_rmem <- \(...) {
  ...
  ###::
  loglines <<- c(loglines, 
                 (paste0("!!:: 15 rmem at ", doing, 
                         " with <a> = ", ..1, 
                         " (unaltered param) and <b> = ", b, 
                         " (read_loc(use_integer(", ..2, 
                         ")) calling write_loc() follows.")),
                 "")
  write_loc(value = b, to = a)
  finish(doing, 2)
...
op_17_call <- \(...) {
  ...
  ###:: 
  loglines <<- c(loglines, 
                 (paste0("!!:: 17 call called by doing = ",
                         doing, " with <a> = ", a, 
                         "(use_integer(", ..1, ")).")))
...
op_20_in <- \(...) {
  ...
  ###::
  loglines <<- c(loglines, (paste0("!!:: opcode 20 in received this string input: {", str, "} and is now parsing.")))
  raw_str <- charToRaw(str)
  loglines <<- c(loglines, "!!:: opcode 20 in found these values:", "")
  cat(raw_str)
